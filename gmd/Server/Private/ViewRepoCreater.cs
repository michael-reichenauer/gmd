...
{
    Repo GetViewRepoAsync(
        Repo augRepo,
        IReadOnlyList<string> showBranches,
...
class ViewRepoCreater : IViewRepoCreater
{
    ...
    public Repo GetFilteredViewRepoAsync(Repo repo, string filter, int maxCount)
    {
        using (Timing.Start($"Filtered repo on '{filter}'"))
        {
            ...
            var viewCommits = filteredRepo.ViewCommits.Where(c => filteredCommitsById.ContainsKey(c.Id)).ToList();
    ...
    static void SetAheadBehind(List<Branch> filteredBranches, List<Commit> filteredCommits)
    {
    ...
    static void SetBehindCommits(Branch remoteBranch, List<Branch> filteredBranches, List<Commit> filteredCommits)
    {
        ...
        while (commitIndex != -1)
        {
            ...
            // Commit is(behind)
            filteredCommits[commitIndex] = commit with
            {
                IsBehind = true,
            };
    ...
    static void SetAheadCommits(Branch localBranch, List<Branch> filteredBranches, List<Commit> filteredCommits)
    {
    ...
    List<Branch> FilterOutViewBranches(
        Repo repo,
        IReadOnlyList<string> showBranches,
        ShowBranches show = ShowBranches.Specified,
        int count = 1
    )
    {
        ...
        switch (show)
        {
            ...
            case ShowBranches.AllRecent:
                repo.AllBranches.Where(b => !b.IsCircularAncestors)
                    ...
                    .Concat(
                        showBranches
                            .Select(n => repo.BranchByName.TryGetValue(n, out var bbb) && bbb.IsInView ? bbb : null)
                            .Where(b => b != null)
                    )
                    .ForEach(b => AddBranchAndAncestorsAndRelatives(repo, b, branches));
                break;
            case ShowBranches.AllActive:
                repo.AllBranches.Where(b => !b.IsCircularAncestors && b.IsGitBranch && b.IsPrimary)
                    .ForEach(b => AddBranchAndAncestorsAndRelatives(repo, b, branches));

                break;
            case ShowBranches.AllActiveAndDeleted:
                repo.AllBranches.Where(b => !b.IsCircularAncestors && b.IsPrimary)
                    .ForEach(b => AddBranchAndAncestorsAndRelatives(repo, b, branches));
                break;
        }
        ...
        if (detached != null)
            AddBranchAndAncestorsAndRelatives(repo, detached, branches);
    ...
    void AddBranchAndAncestorsAndRelatives(Repo repo, Branch? branch, IDictionary<string, Branch> branches)
    {
        ...
        primary.RelatedBranchNames.ForEach(n =>
            AddBranchAndAncestorsAndRelatives(repo, repo.BranchByName[n], branches)
        );
    }
    ...
    static int CompareBranches(Branch b1, Branch b2, List<BranchOrder> branchOrders)
    {
        if (b1 == b2)
            return 0;
        if (b1.Name == b2.ParentBranchName)
            return -1; // b1 is parent of b2
        if (b2.Name == b1.ParentBranchName)
            return 1; // b2 is parent of b1

        if (b2.AncestorNames.Contains(b1.Name))
            return -1; // b1 is ancestor of b2
        if (b1.AncestorNames.Contains(b2.Name))
            return 1; // b2 is ancestor of b1
