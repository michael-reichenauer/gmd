...
class RepoWriter : IRepoWriter
{
    ...
    public IEnumerable<Text> ToPage(
        IViewRepo repo,
        int firstRow,
        int count,
        int currentIndex,
    ...
    {
        if (!repo.Repo.ViewCommits.Any() || count == 0)
            return new List<Text>();
        if (!selection.IsEmpty)
            currentIndex = selection.InitialIndex;
    ...
    static void WriteSubjectColumn(
        TextBuilder text,
        Columns cw,
        Commit c,
        Branch currentRowBranch,
    ...
    )
    {
        ...
        { // There is room for subject, tags and tips, check if shorten is needed
            ...
            { // Need to shorten tags and tips a bit too, but give tags and tips at least each 3 chars and max 20
                subjectWidth = Math.Min(columnWidth - 3 - 3, columnWidth - Math.Min(tagsWidth + tipsWidth, 20));
    ...
    static Text GetSubjectText(Commit c, Branch currentRowBranch)
    {
        ...
        if (c.Id == Repo.EmptyRepoCommitId)
        {
            text.Dark(c.Subject);
        }
        else if (c.IsConflicted)
        {
            text.BrightRed(c.Subject);
        }
        else if (c.IsUncommitted)
        {
            text.BrightYellow(c.Subject);
        }
        else if (c.IsAhead)
        {
            text.BrightGreen(c.Subject);
        }
        else if (c.IsBehind)
        {
            text.BrightBlue(c.Subject);
        }
        else if (c.Id == Repo.TruncatedLogCommitId)
        {
            text.Dark(c.Subject);
        }
        else if (c.BranchPrimaryName == currentRowBranch.PrimaryName)
        {
            text.White(c.Subject);
        }
        else
        {
            text.Dark(c.Subject);
    ...
    static Text GetTagsText(Commit c) =>
        c.Tags.Any() ? Text.Green($"[{string.Join("][", c.Tags.Select(t => t.Name))}]") : Text.Empty;
