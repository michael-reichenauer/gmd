...
class DiffView : IDiffView
{
    ...
    Text ToDiffRowText(DiffRow row, int index, int columnWidth, int viewWidth)
    {
        ...
        return Text
            .Add(isHighlighted && IsSelectedLeft ? left.ToSelect() : left)
            .Add(splitLineChar)
            .Add(isHighlighted && !IsSelectedLeft ? right.ToSelect() : right);
    }
