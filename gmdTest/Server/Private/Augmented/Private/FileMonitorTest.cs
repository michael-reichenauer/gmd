...
public class FileMonitorTest
{
    ...
    // When a change was made is the file's own modification time, not when the event came, which
    // is later by however long the file system took to tell. That difference is what decides
    // whether a read of the repo saw the change (RepoView.OnRefreshRepo).
    [TestMethod]
    public void TestAChangeIsDatedByTheFileNotByTheEvent()
    {
        using var folder = new TempFolder();
        var file = folder.File("main", new DateTime(2026, 1, 1, 11, 0, 0, DateTimeKind.Utc));

        monitor.RepoChange(file, "refs/heads/main", WatcherChangeTypes.Changed);
        now += pastDelay;
        mainThread.Tick();

        Assert.AreEqual(new DateTime(2026, 1, 1, 11, 0, 0, DateTimeKind.Utc), repoEvents[0].ChangedAt);
        Assert.AreEqual(startTime, repoEvents[0].TimeStamp, "Reported when the event came");
    }

    // A file that is gone is dated by its folder, which its entry going changed
    [TestMethod]
    public void TestADeletedFileIsDatedByItsFolder()
    {
        using var folder = new TempFolder();
        var folderTime = new DateTime(2026, 1, 1, 11, 30, 0, DateTimeKind.Utc);
        Directory.SetLastWriteTimeUtc(folder.Path, folderTime);

        monitor.RepoChange(Path.Join(folder.Path, "gone"), "refs/heads/gone", WatcherChangeTypes.Deleted);
        now += pastDelay;
        mainThread.Tick();

        Assert.AreEqual(folderTime, repoEvents[0].ChangedAt);
    }

    // A burst is dated by the latest change in it, whatever order the events came in, since that is
    // the one a read has to have seen for the burst to be seen
    [TestMethod]
    public void TestABurstIsDatedByItsLatestChange()
    {
        using var folder = new TempFolder();
        var newer = folder.File("a", new DateTime(2026, 1, 1, 11, 50, 0, DateTimeKind.Utc));
        var older = folder.File("b", new DateTime(2026, 1, 1, 11, 10, 0, DateTimeKind.Utc));

        monitor.FileChange(newer);
        monitor.FileChange(older);
        now += pastDelay;
        mainThread.Tick();

        Assert.AreEqual(new DateTime(2026, 1, 1, 11, 50, 0, DateTimeKind.Utc), fileEvents[0].ChangedAt);
    }

    // Nothing on disk to ask, as for the made-up paths above, is dated by the event
    [TestMethod]
    public void TestAChangeWithNothingOnDiskIsDatedByTheEvent()
    {
        monitor.RepoChange(refPath, "refs/heads/main", WatcherChangeTypes.Changed);
        now += pastDelay;
        mainThread.Tick();

        Assert.AreEqual(startTime, repoEvents[0].ChangedAt);
    }

    // A folder of files with the modification times a test gives them, deleted afterwards
    sealed class TempFolder : IDisposable
    {
        public string Path { get; } =
            System.IO.Path.Join(System.IO.Path.GetTempPath(), $"gmdTest-monitor-{Guid.NewGuid():N}");

        public TempFolder() => Directory.CreateDirectory(Path);

        public string File(string name, DateTime modified)
        {
            var path = System.IO.Path.Join(Path, name);
            System.IO.File.WriteAllText(path, "");
            System.IO.File.SetLastWriteTimeUtc(path, modified);
            return path;
        }
